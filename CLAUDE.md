...
The live browser-only presentation deck for **Taproom Trivia at Fertile Ground**. Private repo, deployed to Cloudflare Pages at `fertile-ground-trivia.pages.dev`.

Forked from `~/Workspace/pub-trivia-scaffold` (itself descended from `star-wars-trivia-game`). The scaffold stays on GitLab and remains the source-of-truth that `/new-pub-trivia-deck` clones.

**This repo is a deck, not a scaffold.** `/new-pub-trivia-deck` must never target it. The Fertile Ground palette, venue logo (`public/logo-fgbc-red.png`), and slide copy are the product — they are not theme-leak points to be replaced. Engine improvements worth sharing get ported back to the scaffold by hand; there is no automatic sync.
...
The two windows talk via `BroadcastChannel` (channel name `fertile-ground-trivia` — distinct from the scaffold's, so this deck and a scaffold deck can run side-by-side without messages crossing). See `src/broadcast.js`. Message types in use:
...
## Fertile Ground branding

These are the venue-branding sites. In the scaffold they are theme-leak anchors that `/new-pub-trivia-deck` rewrites; **here they are the product** — intentional content, not placeholders.

| File | Branding |
|---|---|
| `index.html` | `<title>Taproom Trivia · Fertile Ground</title>`; the inline `<style>` mirrors the palette (`#1A2A4A` bg, `#F2E8CF` text, `#E2A828` selection) |
| `src/meta.js` `DEFAULT_META.title` | `eyebrow: "Presented at Fertile Ground"`, `edition: "Taproom Trivia"`, `tagline`, `hosts`, `footerDate` — hosts override at runtime via the Edit Questions tab |
| `src/meta.js` `DEFAULT_META.nextEvent` | `venue: "Fertile Ground"` + per-event fields |
| `src/slides.jsx` `PALETTE` | navy `#1A2A4A` / cream `#F2E8CF` / rust `#C8201E` / gold `#E2A828` |
| `src/slides.jsx` `Logo` | `public/logo-fgbc-red.png`, `alt="Fertile Ground Beer Co"` — on Title/End; `onError` hides it if the PNG goes missing |
| `src/slides.jsx` | `FERTILE GROUND GIFT CARD` prize-slide copy; `left="Fertile Ground"` FooterBar on the title slide |
| `src/App.jsx` | `DEFAULT_ACCENT = "accent-gold"` |

Identifiers namespaced to this deck (all `fertile-ground-trivia`): the `package.json` name, `CHANNEL_NAME` in `broadcast.js`, the four `localStorage` keys, and `QUESTIONS_EXPORT_TYPE`. Because the export type is namespaced, deck JSON exported from the scaffold or a themed sibling will **not** import here — retag its `type` field first.
...
Private GitHub repo `michaellambgelo/fertile-ground-trivia`, auto-deployed to **Cloudflare Pages** on every push to `main` via the GitHub git integration. Project `fertile-ground-trivia`, build command `npm run build`, output directory `dist`, production branch `main`. Live at `https://fertile-ground-trivia.pages.dev`; pull requests get preview deployments.

Because Pages serves from the domain root, `base` is `/` in `vite.config.js` (the scaffold uses a `/pub-trivia-scaffold/` subpath for GitLab Pages — don't copy that value back). Image fallbacks in `src/pictures.js` and `LOGO_SRC` in `slides.jsx` both go through `import.meta.env.BASE_URL`, so they follow `base` in dev and prod alike. `.node-version` pins the Pages build image to Node 20.

`scripts/deploy.sh` (used by `/deploy`) is just `git push origin main` plus the contract's `::deploy:` lines — it emits no `watch=` line, because the build runs in Cloudflare, not GitHub Actions. There is no wrangler config: the git integration owns the deploy, so `wrangler pages deploy` is not part of the flow.
